...
contract A {
    ...
    function bit_xor(IKosVariable4V memory self, IKosVariable4V memory rhs) internal pure returns (IKosVariable4V memory) {
        uint32[OZKB_PUBLIC_BRANCHES] memory val;
        for (uint i = 0; i < OZKB_PUBLIC_BRANCHES; ++i) {
    ...
    function _sum(IKosVariable4V[] memory input, uint32[] memory in_pub) internal pure returns (IKosVariable4V[] memory) {
        IKosVariable4V[] memory out = new IKosVariable4V[](1);
        out[0] = add_op(add_op(input[0],input[1]),input[2]);
        return out;
    }

    function sum(VerifyingProof memory proof) public returns (uint256) {
        if (ZKBoo_verify(proof)) { return proof.output[0]; } else { return 0; }
    }
    ...
    function ZKBoo_verify(VerifyingProof memory proof) internal pure returns (bool) {
        ...
        for (uint round = 0; round < OZKB_NUMBER_OF_ROUNDS; round++) {
            ...
            IKosVariable4V[] memory ikos_output = _sum(ikos_input, proof.input_pub);
